...
# packages required
packages <- c("qiime2R", "phyloseq", "tidyverse", "magrittr", "devtools", "qiime2R", "here", "breakaway", "DivNet", "openxlsx", "ape", "vegan", "ggtext", "cowplot", "RColorBrewer", "microbiome", "lme4", "lmerTest","decontam", "remotes", "ampvis2", "speedyseq", "ggh4x", "kableExtra", "phia", "MASS", "ggord", "glue", "lubridate", "data.table")
...
sample_data(psdata_RBRA)$day_DOC <- factor(sample_data(psdata_RBRA)$day_DOC, levels = c("14_H", "42_H_D",	"70_D",	"98_D_A",	"126_A",	"154_NH4",	"14_Nitrate_red",	"70_Microaerob", "126_Microaerob",	"154_Microaerob"))
...
sample <- subset_samples(physeq3, mock_community == "no")
...
rel_sample <- transform_sample_counts(sample, fun = function(x) x/sum(x)) ## samples

# MOCK ASV IN SAMPLE
filter_0.005_m <- phyloseq::genefilter_sample(rel_mock, filterfun_sample(function(x) x > 0.005))

top_mock_in_sample <- prune_taxa(filter_0.005_m, rel_sample)

top_mock_in_sample_1 = 
  data.frame(top_mock_in_sample@otu_table) %>% 
  as_tibble(rownames = "ASV") %>% 
  pivot_longer(names_to = "samples", cols = -ASV) %>% 
  mutate(type="sample")

top_mock_in_mock_1 = 
  data.frame(rel_mock@otu_table) %>% 
  as_tibble(rownames = "ASV") %>% 
  filter(ASV %in% taxa_names(top_mock_in_sample)) %>% 
  pivot_longer(names_to = "samples", cols = -ASV) %>% 
  mutate(type="mock") 

top_mock_long = bind_rows(top_mock_in_sample_1, top_mock_in_mock_1)
mock_taxa_names = data.frame(top_mock_in_sample@tax_table) %>% as_tibble(rownames="ASV")
  
top_mock_plot <- top_mock_long %>% 
  group_by(ASV, type) %>% 
  summarise(mean = mean(value)) %>% 
  inner_join(., mock_taxa_names, by = "ASV") %>% 
  pivot_wider(names_from = "type", values_from = "mean", values_fill = 0) %>%
  unite(ASV, Species, col = "ASV_Species", sep = "_", remove = FALSE) %>%
  ggplot(aes(x=mock, y=sample)) +
  geom_point(aes(color = ASV_Species))

top_mock_plot

# SAMPLE ASV IN MOCK
filter_0.15_s <- phyloseq::genefilter_sample(rel_sample, filterfun_sample(function(x) x > 0.15))

top_sample_in_mock <- prune_taxa(filter_0.15_s, rel_mock)

top_sample_in_mock_1 = 
  data.frame(top_sample_in_mock@otu_table) %>% 
  as_tibble(rownames = "ASV") %>% 
  pivot_longer(names_to = "samples", cols = -ASV) %>% 
  mutate(type="mock")

top_sample_in_sample_1 = 
  data.frame(rel_sample@otu_table) %>% 
  as_tibble(rownames = "ASV") %>% 
  filter(ASV %in% taxa_names(top_sample_in_mock)) %>% 
  pivot_longer(names_to = "samples", cols = -ASV) %>% 
  mutate(type="sample") 

top_sample_long = bind_rows(top_sample_in_mock_1, top_sample_in_sample_1)
sample_taxa_names = data.frame(top_sample_in_mock@tax_table) %>% as_tibble(rownames="ASV")
  
top_sample_plot <- top_sample_long %>% 
  group_by(ASV, type) %>% 
  summarise(mean = mean(value)) %>% 
  inner_join(., sample_taxa_names, by = "ASV") %>% 
  pivot_wider(names_from = "type", values_from = "mean", values_fill = 0) %>% 
  ggplot(aes(x=mock, y=sample)) +
...
#RITA'S INITIAL PROCESSING, but now with relative abundances
...
not_rar_0.01 <- prune_taxa(filter_0.01,rel_not_rar) #11 OTU, all these taxa correspond to taxa that should be in the mock community (except ebc4facffb9f550d9895b0e2057b26b0, Genus__Burkholderia-Caballeronia-Paraburkholderia) and their relative abundance in the samples (0.000-0.003) is much lower than in the mock community samples (>0.010)
...
### 3eff8f2d520a354e57817f9210dc426d (Genus__Lysinibacillus, mock taxa) mock: 0.0042-0.0055; sample: 0.0000
### 599b022f8ad5fc64b1423b0931dd052e (Bacillus_coagulans, mock taxa) mock: 0.0044-0.0100; sample: 0.0000
### 500d294e65ac031e205edb2ea3b9ddf6 (Family_Comamonadaceae, not mock taxa) mock: 0.0043-0.0073; sample: up to 0.21
### eee4b43e99303cb0092fe501da59d116 (Genus__Clostridium_sensu_stricto_1, not mock taxa) mock: 0.0077-0.0087; sample: up to 0.42
...
psdata_RBRA <- prune_taxa(not_mock_taxa_2, not_mock_taxa_1)
```
...
psdata_RBRA_rel <- transform_sample_counts(psdata_RBRA, fun = function(x) x/sum(x)) # 2106 taxa

# total reads left = 1998811
...
psdata_RBRA_0.01pct <- prune_taxa(taxa_sums(psdata_RBRA_rel) > 0.0001, psdata_RBRA) # 1664 taxa (99.93% abundance)
...
min_cover <- min(sample_sums(psdata_RBRA)) #32198

# rarefaction curves plot
breakscolumn <- c("Nitrate_red_Microaer", "Nitrate_red", "Aerob", "Aerob_Microaer")
...
amp_rarecurve(data = data, 
  ...
  geom_vline(xintercept = 32198, size = 0.25)
...
**Figure S1**. Rarefaction curves. Minimum coverage was 32198 reads per sample; a depth at which full diversity is captured. Hence, no samples were dropped in later stages.<br>

### Alpha diversity

```{r alpha diversity, message=F, echo=T, eval=T, warning=T, include=T, cache=T, fig.width=6, fig.height=6}
# data rarefied to 32198 r/s
set.seed(711)

ps_32198 <- rarefy_even_depth(psdata_RBRA, 32198, rngseed = 711)

# calculate richness
metadata <- data.frame(sample_data(ps_32198))
alpha <- estimate_richness(ps_32198, measures = c("Chao1", "Shannon"))

alpha$sampleid <- sample_names(ps_32198)
alpha_data <- 
...
chao1 <- alpha_data %>% 
  ...
  scale_fill_manual(values = c("#B2182B", "#F4A582", "#92C5DE", "#2166AC", "#053061")) +
...
ggsave("C:/Users/Rita/Documents/R/amendment_micropollutant_biodegradation_aquifer/figures/Figure4_alpha.pdf", width = 6, height = 6)
```

**Figure Alpha**. Microbial community diversity across the different columns in time. Alpha diversity expressed as A) estimated ASV richness (Chao1); B) estimated ASV diversity (Shannon).<br>

### Beta diversity

Due to no replicates no PERMANOVA was performed.

```{r total beta diversity, message=F, echo=T, eval=T, warning=T, include=T, cache=F, fig.width=8, fig.height=8}
# relative abundance data
...
plot_PCoA_BC <- plot_ordination(physeq = psdata_RBRA_rel, 
  ...
  scale_color_manual(name = "Column_height",
                     values = c("#B2182B", "#F4A582", "#92C5DE", "#2166AC", "#053061")) +
  scale_shape_manual(name = "Day_DOC",
                       values = c(0,6,1,2,5,4,7,10,9,14)) +
  geom_point(size = 2) +
...
plot_PCoA_wu <- plot_ordination(physeq = psdata_RBRA_rel, 
                                 ordination = PCoA_wu, 
                                 type = "samples", 
                                 axes = c(1,2), 
                                 shape = "day_doc",
...
# combined plot
PCoA = plot_grid(plot_PCoA_BC + theme(legend.position = "none"),
                 plot_PCoA_Jac + theme(legend.position = "none"),
                 plot_PCoA_uu + theme(legend.position = "none"),
                 plot_PCoA_wu + theme(legend.position = "none"),
                   labels  = c("A", "B", "C", "D"),
                   ncol = 2,
                   nrow = 2,
                   align = "hv")
...
beta_plot <- plot_grid(PCoA, legend_beta, ncol=2, rel_widths = c(5, 1))

beta_plot

ggsave("C:/Users/Rita/Documents/R/amendment_micropollutant_biodegradation_aquifer/figures/Figure4_beta.pdf", width = 8, height = 8)
```

**Figure Beta**. Microbial community diversity across the different columns in time. Principal coordinates analysis (PCoA) ordination plots of community composition based on A) Bray-Curtis; B) Jaccard; C) unweighted UniFrac distances; and D) weighted UniFrac, respectively.<br>

#### Distances between consecutive electron donors

```{r ditances electron donors, message=F, echo=T, eval=T, warning=F, include=T, cache=F, fig.width=8, fig.height=8}
...
comp_donor = c("14_H_to_42_H_D", "42_H_D_to_70_D", "70_D_to_98_D_A", "98_D_A_to_126_A", "126_A_to_154_NH4")
...
comp_inoc_bc <- all_dists %>%
  ...
  scale_color_manual(values = c("#F4A582", "#92C5DE", "#2166AC", "#053061")) +
  scale_fill_manual(values = c("#F4A582", "#92C5DE", "#2166AC", "#053061")) +
...
# plot Jall distances between e-donor
comp = plot_grid(comp_inoc_bc + theme(legend.position = "none"),
...
comp_plot <- plot_grid(comp, legend_comp, ncol=2, rel_widths = c(5, 1))

comp_plot

ggsave("C:/Users/Rita/Documents/R/amendment_micropollutant_biodegradation_aquifer/figures/Figure4_distance_e_donor.pdf", width = 8, height = 8)
```

**Figure distance e donor**. A) Bray Curtis, B) Jaccard, C) unweighted UniFrac and D) weighted UniFrac distances between microbial communities after each electron donor amendment per column with electron donor amendment.<br>

#### Distances between consecutive microaeration periods

```{r ditances microaeration periods, message=F, echo=T, eval=T, warning=F, include=T, cache=F, fig.width=8, fig.height=8}
# remove column no electon donor amendment
...
# define consecutive sampling moments
comp_microaerob = c("14_Nitrate_red_to_70_Microaerob", "70_Microaerob_to_126_Microaerob", "126_Microaerob_to_154_Microaerob")
...
# plot Jall distances between e-donor
comp2 = plot_grid(comp_inoc_bc2 + theme(legend.position = "none"),
...
comp_plot2 <- plot_grid(comp2, legend_comp2, ncol=2, rel_widths = c(5, 1))

comp_plot2

ggsave("C:/Users/Rita/Documents/R/amendment_micropollutant_biodegradation_aquifer/figures/Figure4_distance_microaerob.pdf", width = 8, height = 8)
```

**Figure distance redox condition**. A) Bray Curtis, B) Jaccard, C) unweighted UniFrac and D) weighted UniFrac distances between microbial communities in column A with electron donor amendment.<br>
...
plot_Class <- inner_join(Class_abundances, Class_pool, by="Class") %>% 
  ...
  mutate(Class = factor(Class), 
         ...
         redox_type = fct_relevel(day_doc, rev(c("14_H", "42_H_D",	"70_D",	"98_D_A",	"126_A",	"154_NH4",	"14_Nitrate_red",	"70_Microaerob", "126_Microaerob",	"154_Microaerob"))),
...
ggplot(aes(x=day_doc, y = column_height, fill = Class)) +
  geom_col() +
  ...
  facet_wrap(~column_height, ncol = 5, scales = "free_x") +
